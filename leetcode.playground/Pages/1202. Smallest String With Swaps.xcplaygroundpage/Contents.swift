...
class Solution {
    func smallestStringWithSwaps(_ s: String, _ pairs: [[Int]]) -> String {
        ...
        var ans = Array(s), dsu: [Int: Set<Int>] = [:], newPairs: [[Int]] = []
        for pair in pairs {
            if dsu[pair[0]] == nil {
                dsu[pair[0]] = [pair[1]]
            } else {
        ...
        for key in keys {
            ...
            if !connection.isEmpty {
                newPairs.append(Array(connection).sorted(by: <))
            }
        }
        for pair in newPairs {
            var array: [Character] = []
            for position in pair {
                array.append(characters[position])
            }
            array.sort(by: <)
    ...
    private func connect(_ key: Int, _ connection: inout Set<Int>, _ dsu: inout [Int: Set<Int>]) {
        guard let set = dsu[key] else {
            return
        }
        connection.insert(key)
        dsu.removeValue(forKey: key)
        for newKey in set {
            connect(newKey, &connection, &dsu)
        }
    }
    
}

let s = "dcab", pairs = [[0,3],[1,2],[0,2]]
let solution = Solution()
let ans = solution.smallestStringWithSwaps(s, pairs)
ans
