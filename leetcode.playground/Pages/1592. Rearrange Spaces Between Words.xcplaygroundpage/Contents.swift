...
class Solution {
    func reorderSpaces(_ text: String) -> String {
        var text = Array(text), spaceCount = 0, wordCount = 0, isWord = false
        ...
        if spaceCount > 0 {
            text.append(contentsOf: Array<Character>(repeating: Character(" "), count: spaceCount))
        }
        if spaceCount < 0 {
            text.removeLast(-spaceCount)
        }
        return String(text)
    }
}
...
let ans = solution.reorderSpaces(text)
ans
