...
class ThroneInheritance {
    ...
    func birth(_ parentName: String, _ childName: String) {
        let child = Tree(childName)
        dict[parentName]?.children.append(child)
    ...
    func getInheritanceOrder() -> [String] {
        let order: [String] = recursive(self.root)
        return order
    }
    
    private func recursive(_ node: Tree?) -> [String] {
        guard let node = node else {
            return []
        }
        let cur: [String] = node.alive ? [node.name] : []
        
        return cur + node.children.map { self.recursive($0) }.reduce([], +)
    }
}
/**
